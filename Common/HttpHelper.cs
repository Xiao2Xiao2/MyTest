...
namespace Common
{
    ...
    public class HttpHelper
    {
                ...
                using (response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    if (ResponseByte != null & ResponseByte.Length > 0)
                    {
                        ...
                        if (encoding == null)
                        {
                            ...
                            if (c.Length > 2)
                                encoding = Encoding.GetEncoding(c.Trim().Replace("\"", "").Replace("'", "").Replace(";", "").Replace("iso-8859-1", "utf-8"));
                            else
                            {
                                if (string.IsNullOrWhiteSpace(response.CharacterSet)) encoding = Encoding.UTF8;
        ...
        {
            //验证在得到结果时是否有传入数据
            if (request.Method.Trim().ToLower().Contains("post") || request.Method.Trim().ToLower().Contains("put"))
            {
