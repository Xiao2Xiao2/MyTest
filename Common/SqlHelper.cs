...
{   /// <summary>
    ...
    public class SqlHelper
    {
        ...
        public void AddCondition(string _FieldName, Operators _Operators, DataType _DataType, string _Value,
                                 bool _isMargeOr)
        {
            // 如果值为空则直接返回，无需组合条件进行查询，优化性能
            if ((_Operators != Operators.IsNull &&
                _Operators != Operators.IsNotNull)
        ...
        /// <param name="_Value">比较值</param>
        /// <param name="_isMargeOr">是否加入到OR组合条件中去</param>
        /// <param name="textEmptyisReturn">如果条件为空，是否允许追加空条件</param>
        public void AddCondition(string _FieldName, Operators _Operators, DataType _DataType, string _Value,
                                 bool _isMargeOr, bool textEmptyisReturn)
        {
            if (!textEmptyisReturn)
                return;
            string isMargeOr = _isMargeOr.ToString();
            AddCondition("", _FieldName, _Operators, _DataType, _Value, isMargeOr);
        }
        ...
        public string GetFilterCondition()
        {
            ...
            {
                //sqlString += sqlString.Length > 0 ? " OR (" + orBuilder.ToString().Substring(4).Replace("AND", "OR") + ")" : " (" + orBuilder.ToString().Substring(4).Replace("AND", "OR") + ")";

                // AND条件组与OR条件组应使用AND连接 
        ...
        private string ConvertRangeType(Operators range)
        {
            ...
            switch (range)
            {
                ...
                case Operators.NotEqual:
                    return " <> ";
                case Operators.In:
                case Operators.SubQueryIn:
                    return " IN ";
                case Operators.SubQueryNotIn:
                    return " NOT IN ";
                case Operators.Like:
                case Operators.LikeLeft:
                    return " LIKE ";
                ...
                default:
                    // 无效的操作符
                    throw new ArgumentOutOfRangeException("range", "Invalid operator value.");
            }
        }
    }
